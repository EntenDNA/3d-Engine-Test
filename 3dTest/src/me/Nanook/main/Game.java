...
public class Game implements Runnable{
	...
	private void tick()
	{
		// so its only 60 ticks per second
            ...
            if(delta >=1) 
            {
        		...
        		for(Cube cube : cubeList)
        		{
        			for (triangle tri : cube.getMesh().tris)
        			{
        				...
        				// transform triangle
        				triTransformed = new triangle(mathHelper.multiplyVectorMatrix(tri.vec3dList[0], matWorld),
        											  mathHelper.multiplyVectorMatrix(tri.vec3dList[1], matWorld),
        											  mathHelper.multiplyVectorMatrix(tri.vec3dList[2], matWorld));
        				...
        				if(mathHelper.vectorDotProduct(normal, vCameraRay) < 0.0f)
        				{
        					...
        					triViewed = new triangle(mathHelper.multiplyVectorMatrix(triTransformed.vec3dList[0], matView),
        										     mathHelper.multiplyVectorMatrix(triTransformed.vec3dList[1], matView),
        										     mathHelper.multiplyVectorMatrix(triTransformed.vec3dList[2], matView));
        					...
        					{
        						// project from 3d -> 2d	
        						triProjected = new triangle(mathHelper.multiplyVectorMatrix(clipped[i].vec3dList[0], matProj),
        											   		mathHelper.multiplyVectorMatrix(clipped[i].vec3dList[1], matProj),
        											   		mathHelper.multiplyVectorMatrix(clipped[i].vec3dList[2], matProj));
        		...
        		if(this.showDebug)
        		{
        			...
        			g.drawString("Pitch: " + Math.round(fPitch*180/Math.PI * 100) / 100f, 0, 60);
        		}
